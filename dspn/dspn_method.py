from dspn_base import StructuredModel
...
#from pystruct.utils import loss_augment_unaries
...
class Method(object):
    ...
    def getDataTrainTestTemp(self, path, trainNum, testNum):
        file1 = open(path, 'r') 
        lineNum = 1
        X_train = []
        Y_train = []
    ...
    def testFunction(self, model, testNum,thread,X_test,Y_test, Y_pred, infTimes, random_pred = False):
        ...
        #print(influence_X)
        #print(influence_Y)
        #print(influence_Y_pred)
        #input("Press Enter to continue...")
        #os.system("taskset -p 0xff %d" % os.getpid())
        #instance.testInfluence_0(X_test[0],{},1024*1024,thread)
        
        '''
        p = multiprocessing.Pool(thread)
        influence_X = p.starmap(instance.testInfluence_0, ((x,{}, infTimes, 1) for x in X_test ))
        p.close()
        p.join()
        
        p = multiprocessing.Pool(thread)
        influence_Y = p.starmap(instance.testInfluence_0, ((x, y, infTimes, 1) for x, y in zip(X_test, Y_test) ))
        p.close()
        p.join()
        
        p = multiprocessing.Pool(thread)
        influence_Y_pred = p.starmap(instance.testInfluence_0, ((x, y_pred, infTimes, 1) for x, y_pred in zip(X_test, Y_pred) ))
        p.close()
        p.join()
        '''
...
class SocialGraph(object):
    ...
    def __init__(self, path, vNum, modelType):
        self.nodes={}
        self.vNum = vNum
        ...
        while len(actTime)>0:
            ...
            if current_node not in fstate:
                if current_node_time != tTime[current_node]:
                    sys.exit("current_node_time != tTime[current_node], {} {}".format(current_node_time, tTime[current_node])) 
...
class DiffusionGraph(object):
    ...
    def __init__(self, path_graph, path_distance, vNum):
        ...
        for node in self.nodes:
            neighbor = {}
            self.distance[node]=neighbor
            #print(node)
            #print(node.index)
            #input("Press Enter to continue...")
            _,cover = self.spread({node},{}, getCover=True)
            for node_to in self.nodes:
                if node_to in cover:
                    self.distance[node][node_to]=cover[node_to]   
         '''        
...
class InputInstance(object):
    ...
    def __init__(self, socialGraphPath, featurePath, featureNum, vNum, effectAreaHotNum, modelType, balance_para, loss_type, 
                 featureRandom = False, maxFeatureNum = 500, thread = 1, LAI_method = None,featureIndexes=None):
        #self.graphs=[];
        self.featureNum = featureNum
        self.vNum = vNum
        self.socialGraph = SocialGraph(socialGraphPath, vNum, modelType);
...
"""
    def __init__(self, input_instance):
        ...
        self.feature_num = len(input_instance.graphs)
    ...
    def joint_feature(self, x, y, input_instance):
        feature = np.zeros(self.feature_num)
        index = 0
        for graph in input_instance.graphs:
            distance_matrix = np.zeros( (2, 3) )
            for v in range(intput_instance):
                x_min=sys.maxsize
                for u in x:
                    if distance_matrix[v][u]<x_min:
                        x_min=distance_matrix[v][u]
